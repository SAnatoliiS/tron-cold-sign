...
function formatHumanSummary(rawData) {
	...
	for (let i = 0; i < contracts.length; i++) {
		...
		if (type === "TransferContract" && val) {
			...
			const amountSun = Number(val.amount);
			if (!Number.isFinite(amountSun) || amountSun < 0) {
				throw new Error("TransferContract: invalid amount");
			}
			lines.push(`Contract #${i + 1}: TransferContract (TRX)`);
			lines.push(`  From:    ${from}`);
			lines.push(`  To:      ${to}`);
			lines.push(`  Amount:  ${(amountSun / SUN).toFixed(6)} TRX  (${amountSun} SUN)`);
			...
			if (cv !== undefined && cv !== null && Number(cv) > 0) {
				const cvn = Number(cv);
				lines.push(`  TRX with call:    ${(cvn / SUN).toFixed(6)} TRX  (${cvn} SUN)`);
			}
			const data = val.data;
	...
	if (fee !== undefined && fee !== null) {
		lines.push(`Fee limit (fee_limit): ${fee} SUN`);
	} else {
