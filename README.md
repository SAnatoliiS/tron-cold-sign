...
- **`packages/client-offline/`** — Vite + React UI (`tron-cold-sign-offline-client` workspace). Depends on **`@tron-cold-sign/core`** as **`"*"`** (в режиме workspaces npm подставляет локальный `packages/core`; то же можно записать как **`workspace:*`**, если ваш `npm install` принимает этот протокол — npm 7.14+). Production build emits **`packages/client-offline/dist/index.html`** only (single inlined bundle; `public/` is not copied into `dist`).
...
**tsup** (esbuild + `dts`) emits dual **CJS + ESM** from one `src/index.ts` entry, plus **`.d.ts`**, without maintaining a separate browser bundle. The client and Node both resolve the same package. In dev, Vite **does not** pre-bundle `@tron-cold-sign/core` (see `packages/client-offline/vite.config.ts` `optimizeDeps.exclude`) so edits to the workspace package are not stuck behind a stale `node_modules/.vite` cache; other deps are still optimized as usual.

### Client workflow (no manual `build:lib`)

- **`npm run dev:client-offline`** (repo root) runs **`concurrently`**: `tsup --watch` in **`packages/core`** and the Vite dev server in **`packages/client-offline/`**.
- **`npm run build:client-offline`** runs **`vite build`** in the client workspace; the client’s **`prebuild`** runs **`npm run build:core`** so the core package is built before the static bundle. The client uses **`vite-plugin-singlefile`**, so **`packages/client-offline/dist/index.html`** inlines the app JS/CSS (one large HTML). You can open it via **`file://`**; a plain Vite build without that plugin leaves separate **`assets/*.js`** chunks, which **do not load** from `file://` in typical browsers (ES module `src` restrictions).
- **`npm run test:client-offline`** runs Vitest; the client’s **`pretest`** runs **`build:core`** first.
...
While **`npm run dev:client-offline`** is running:
...
3. If the UI still looks stale after a core change, remove the Vite cache **`node_modules/.vite`** (repo root) and restart the dev server (or run `vite` with `--force` once).
...
The app loads a **`buffer`** polyfill before other modules (`packages/client-offline/src/buffer-polyfill.ts`).
...
| `npm run dev:client-offline` | Watch `core` + Vite dev server for the React UI (`packages/client-offline/`). |
| `npm run build:client-offline` | Production static build → `packages/client-offline/dist/` (runs `build:core` via client `prebuild`). |
| `npm run test:client-offline` | Vitest for `packages/client-offline/` (runs `build:core` via client `pretest`). |
| `npm run test:coverage:client-offline` | Vitest with coverage for `packages/client-offline/src` (builds `core` first); reports under `packages/client-offline/coverage/`. |
...
| `npm run integrity:write` / `integrity:check` | Maintain / verify `INTEGRITY.sha256` over tracked project files (includes `packages/client-offline/src` and `packages/core/src`). |
